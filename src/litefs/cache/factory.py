...
class CacheFactory:
    ...
    def create_cache(
        ...
        **kwargs
    ) -> Union[MemoryCache, TreeCache, RedisCache]:
        """
        创建缓存实例
        
        Args:
            backend: 缓存后端类型（memory, tree, redis）
            **kwargs: 缓存配置参数
    ...
    @staticmethod
    def create_from_config(config) -> Union[MemoryCache, TreeCache, RedisCache]:
        """
        从配置对象创建缓存实例
